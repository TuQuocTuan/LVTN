      ...
      <main className="ml-64 pt-24 p-8 w-full flex flex-col h-screen transition-all duration-300">
        ...
        <div className="flex flex-col gap-6 mb-6 flex-shrink-0">
          <div className="flex justify-between items-end">
            <div>
              <h2 className="text-3xl font-black text-gray-900 mb-1 tracking-tight">Quản lý Nguyên vật liệu</h2>
              <p className="text-neutralCustom text-sm font-medium">Theo dõi sát sao định lượng tồn kho và cảnh báo nhập hàng cho bếp Làng MÌXI.</p>
            </div>
            <button
              onClick={openAddModal}
              className="flex items-center gap-2 bg-primary text-white px-6 py-3 rounded-xl font-bold shadow-md hover:bg-secondary transition-all active:scale-95 text-sm cursor-pointer"
            >
        ...
        <div className="flex-1 overflow-hidden">
          <div className="bg-white rounded-2xl border border-neutralCustom/20 overflow-hidden flex flex-col h-full shadow-sm">
            <div className="p-4 bg-stone-50/50 border-b border-neutralCustom/20 flex gap-2 overflow-x-auto no-scrollbar shrink-0">
              {categories.map((cat) => (
                <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-4 py-2 rounded-lg font-black text-xs sm:text-sm transition-all whitespace-nowrap cursor-pointer ${activeCategory === cat ? 'bg-primary text-white shadow-sm' : 'text-neutralCustom hover:bg-white border border-transparent'}`}>
                  {cat}
                </button>
              ))}
            </div>
            ...
            <div className="flex-1 overflow-y-auto relative custom-scrollbar divide-y divide-neutralCustom/10">
              {isLoading ? (
                ...
                paginatedIngredients.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="grid grid-cols-12 gap-4 px-6 py-4 hover:bg-stone-50/60 transition-colors items-center group">
                      <div className="col-span-4 flex items-center gap-4 min-w-0">
                        <div className={`w-12 h-12 rounded-xl bg-stone-100 overflow-hidden border border-neutralCustom/15 shadow-sm shrink-0 ${isOutOfStock ? 'opacity-50 grayscale' : ''}`}>
                          <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                        </div>
                        <div className="min-w-0">
                          <p className={`font-black text-sm truncate ${isOutOfStock ? 'text-neutralCustom/60 line-through' : 'text-gray-900'}`}>{item.name}</p>
                          <p className="text-neutralCustom text-xs mt-0.5 font-semibold">{item.category}</p>
                        </div>
                      </div>

                      <div className="col-span-1 text-gray-900 text-sm font-semibold">{item.unit}</div>

                      <div className="col-span-3 pr-6">
                        <div className="flex flex-col gap-1">
                          <span className={`${status.color} font-black text-xs sm:text-sm`}>{item.quantity} {item.unit}</span>
                      ...
                      <div className="col-span-1 text-neutralCustom text-sm font-bold text-center">{item.min_stock} {item.unit}</div>

                      <div className="col-span-2 text-gray-900 text-sm font-black text-right">
            ...
            {/* Phân trang */}
            {totalPages > 1 && (
              <div className="p-4 bg-stone-50/50 border-t border-neutralCustom/10 flex justify-center items-center text-sm shrink-0">
                <div className="flex gap-2">
                  <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="p-1.5 border border-neutralCustom/20 rounded-lg hover:bg-white text-neutralCustom disabled:opacity-50 flex items-center justify-center bg-white"><span className="material-symbols-outlined text-sm">chevron_left</span></button>
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                    <button key={page} onClick={() => setCurrentPage(page)} className={`px-3 py-1 rounded-lg shadow-sm font-bold text-xs sm:text-sm ${currentPage === page ? 'bg-primary text-white font-bold' : 'hover:bg-white text-neutralCustom border border-neutralCustom/10 bg-white'}`}>{page}</button>
                  ))}
                  <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages || totalPages === 0} className="p-1.5 border border-neutralCustom/20 rounded-lg hover:bg-white text-neutralCustom disabled:opacity-50 flex items-center justify-center bg-white"><span className="material-symbols-outlined text-sm">chevron_right</span></button>
                </div>
              </div>
            )}
